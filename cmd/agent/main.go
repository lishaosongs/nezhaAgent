...
// AgentCliParam Agent 运行时参数。如需添加新参数，记得同时在 service.go 中添加
...
func init() {
	...
	agentCmd.PersistentFlags().BoolVar(&agentCliParam.DisableCommandExecute, "disable-command-execute", true, "禁止在此机器上执行命令")
	agentCmd.PersistentFlags().BoolVar(&agentCliParam.DisableNat, "disable-nat", true, "禁止此机器内网穿透")
	agentCmd.PersistentFlags().BoolVar(&agentCliParam.DisableSendQuery, "disable-send-query", false, "禁止此机器发送TCP/ICMP/HTTP请求")
	agentCmd.PersistentFlags().BoolVar(&agentCliParam.DisableAutoUpdate, "disable-auto-update", true, "禁用自动升级")
	agentCmd.PersistentFlags().BoolVar(&agentCliParam.DisableForceUpdate, "disable-force-update", true, "禁用强制升级")
	...
	_ = agentConfig.Read(filepath.Dir(ex) + "/config.yml")
...
func handleTcpPingTask(task *pb.Task, result *pb.TaskResult) {
	...
	handleHost, port, err := net.SplitHostPort(task.GetData())
	if err != nil {
		result.Data = err.Error()
		return
	}
	ipAddr, err := lookupIP(handleHost)
	if err != nil {
		result.Data = err.Error()
		return
	}
